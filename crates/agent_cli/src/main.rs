...
fn main() {
    ...
    match args[1].as_str() {
        \"run-demo\" => {
            let in_path = arg_value(&args, \"--in\").map(PathBuf::from).unwrap_or_else(|| usage());
            let out_dir = arg_value(&args, \"--out\").map(PathBuf::from).unwrap_or_else(|| usage());

            let raw = read_text_file(&in_path).unwrap_or_else(|e| panic!(\"{e}\"));
            let (final_state, trace) = heal_to_fixpoint(raw, HealConfig::default()).unwrap_or_else(|e| panic!(\"{e}\"));

            let tf = TraceFile { trace: trace.clone(), final_state: final_state.clone() };

            write_json_file(&out_dir.join(\"trace.json\"), &tf).unwrap();
            write_text_file(&out_dir.join(\"final.json\"), &serde_json::to_string_pretty(&final_state).unwrap()).unwrap();
        ...
        \"verify-trace\" => {
            let trace_path = arg_value(&args, \"--trace\").map(PathBuf::from).unwrap_or_else(|| usage());
            let raw = read_text_file(&trace_path).unwrap_or_else(|e| panic!(\"{e}\"));
            let tf: TraceFile = serde_json::from_str(&raw).unwrap_or_else(|e| panic!(\"json decode: {e}\"));
